...
# ─────────────────────────────────────────────
#  STEP 2 — SPLIT
#  NOTE: CatBoost handles all preprocessing internally
...
model = CatBoostRegressor(
    ...
    random_seed=42,
    verbose=0,
    train_dir=cb_tmpdir   # writable temp dir for sandbox
)
...
imp_series = pd.Series(model.get_feature_importance(),
...
fig.suptitle("CatBoost — ZT Prediction", fontsize=14, fontweight="bold")

# Actual vs Predicted
axes[0].scatter(y_test, y_pred, alpha=0.3, color="#E9C46A",
...
bar_colors   = ["#E9C46A", "#DD8452", "#55A868"]
...
axes[2].barh(top15.index[::-1], top15.values[::-1], color="#E9C46A", alpha=0.85, edgecolor="gray")
axes[2].set_xlabel("Importance", fontsize=11)
axes[2].set_title("Top 15 Feature Importances", fontsize=11)

plt.tight_layout()
plt.savefig("/home/sudarshan/Documents/INFORMATICS/ML/catboost_ZT_results.png", dpi=150, bbox_inches="tight")
print("\nPlot saved -> catboost_ZT_results.png")
plt.show()
