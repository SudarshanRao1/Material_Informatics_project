...
============================================================
  ...
  Model  : EXTRA TREES (Extremely Randomized Trees)
...
# ─────────────────────────────────────────────
#  STEP 2 — SPLIT
#  NOTE: ExtraTrees does NOT need feature scaling
#        It is faster than Random Forest because
#        it uses random thresholds instead of optimal splits
# ─────────────────────────────────────────────
...
model = ExtraTreesRegressor(
    ...
    min_samples_leaf=1,
    max_features="sqrt",
    bootstrap=False,       # ExtraTrees uses full dataset per tree (no bootstrap)
    random_state=42,
    n_jobs=-1
)
...
fig.suptitle("Extra Trees — ZT Prediction", fontsize=14, fontweight="bold")

# Actual vs Predicted
axes[0].scatter(y_test, y_pred, alpha=0.3, color="#2A9D8F",
...
bar_colors   = ["#2A9D8F", "#DD8452", "#55A868"]
...
axes[2].barh(top15.index[::-1], top15.values[::-1], color="#2A9D8F", alpha=0.85)
axes[2].set_xlabel("Importance", fontsize=11)
axes[2].set_title("Top 15 Feature Importances", fontsize=11)

plt.tight_layout()
plt.savefig("/home/sudarshan/Documents/INFORMATICS/ML/extratrees_ZT_results.png", dpi=150, bbox_inches="tight")
print("\nPlot saved -> extratrees_ZT_results.png")
plt.show()